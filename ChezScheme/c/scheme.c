...
static INT run_script(const char *who, const char *scriptfile, INT argc, const char *argv[], IBOOL programp) {
  ...
  /* test-scheme-fn by error.d@gmail.com */
  p = S_symbol_value(S_intern((const unsigned char *)"$test-scheme-fn"));
  if (!Sprocedurep(p)) {
    (void) fprintf(stderr,"\n$test-scheme-fn is not bound to a procedure\n");
    S_abnormal_exit();
  }
  ...
  p = boot_call(tc, p, 3);
  /* test-scheme-fn by error.d@gmail.com end */
